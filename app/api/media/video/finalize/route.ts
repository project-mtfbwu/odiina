...
import { finalizeVideoUpload } from "@/lib/database/media";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await finalizeVideoUpload(context.supabase, attachmentId);
    return context.applyAuthState(
    ...
    const code =
      error instanceof Error ? error.message : "video_finalize_failed";
    return context.applyAuthState(
      NextResponse.json(
        { error: "video_finalize_failed", message: safeErrorMessage(code) },
