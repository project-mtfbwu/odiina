...
import { finalizeAudioUpload } from "@/lib/database/media";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await finalizeAudioUpload(context.supabase, attachmentId);
    return context.applyAuthState(
    ...
    const code =
      error instanceof Error ? error.message : "audio_finalize_failed";
    return context.applyAuthState(
      NextResponse.json(
        { error: "audio_finalize_failed", message: safeErrorMessage(code) },
