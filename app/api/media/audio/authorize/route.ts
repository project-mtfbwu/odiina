...
import { authorizeAudioUpload } from "@/lib/database/media";
...
import { authorizeAudioSchema } from "@/lib/validation/media";

export async function POST(request: NextRequest) {
  if (!featureFlags.audioUploads) {
    return NextResponse.json(
      { error: "audio_uploads_disabled" },
      { status: 404 },
    );
  }
  ...
  try {
    ...
    const input = authorizeAudioSchema.parse(await request.json());
    const authorization = await authorizeAudioUpload(context.supabase, input);
    ...
    const code =
      error instanceof Error ? error.message : "audio_authorize_failed";
    return context.applyAuthState(
      NextResponse.json(
        { error: "audio_authorize_failed", message: safeErrorMessage(code) },
