...
export async function finalizeAudioUpload(
  ...
  const { data, error } = await supabase
    .schema("app")
    .rpc("finalize_audio_upload", { p_attachment_id: attachmentId });
  if (error) throw error;
  return (data as { job_id: string; attachment_state: string }[] | null)?.[0];
}
...
export async function finalizeVideoUpload(
  ...
  const { data, error } = await supabase
    .schema("app")
    .rpc("finalize_video_upload", { p_attachment_id: attachmentId });
  if (error) throw error;
  return (data as { job_id: string; attachment_state: string }[] | null)?.[0];
}
